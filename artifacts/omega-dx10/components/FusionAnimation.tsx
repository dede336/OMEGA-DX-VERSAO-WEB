...
export default function FusionAnimation({
  ...
  const baseImage =
    getCharacterImageSource(baseCharacterId);

  const partnerImage =
    getCharacterImageSource(partnerCharacterId);

  const resultImage =
    getCharacterImageSource(resultCharacterId);

  const resultCharacter =
    getCharacter(resultCharacterId);
  ...
  useEffect(() => {
    ...
    Animated.sequence([
      ...
      Animated.parallel([
        Animated.timing(sourcesOpacity, {
          toValue: 0,
          duration: 220,
          useNativeDriver: true,
        }),
